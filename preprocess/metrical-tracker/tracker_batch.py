...
class Tracker(object):
        ...
        for idx in range(len(frame_ids)):
            ...
            frame = {
                'flame': {
                    ...
                    'tex': self.tex[idx:idx+1].clone().detach().cpu().numpy(),
                    'sh': self.sh[idx:idx+1].clone().detach().cpu().numpy(),
    ...
    def create_parameters(self):
        ...
        self.sh = nn.Parameter(torch.zeros(bz, 9, 3).float().to(self.device))                                   # (1, 9, 3)
    ...
    def create_batch_parameters_from_init(self):
        ...
        # 纹理
        self.tex = nn.Parameter(self.tex.expand(bz, -1))
        self.sh = nn.Parameter(self.sh.expand(bz, -1, -1))
    ...
    def optimize_color(self, batch, pyramid, params_func, pho_weight_func, reg_from_prev=False):
        ...
        for k, level in enumerate(pyramid):
            ...
            tex = self.get_param('tex', params)         # (B, 140)
    ...
    def checkpoint(self, batch, visualizations=[[View.GROUND_TRUTH, View.LANDMARKS, View.HEATMAP], [View.COLOR_OVERLAY, View.SHAPE_OVERLAY, View.SHAPE]], frame_dst='video', save=True, dump_directly=False, k=0):
        ...
        with torch.no_grad():
            ...
            shape_masks = ((ops['alpha_images'] * ops['mask_images_mesh']) > 0.).int()
                ...
                for views in visualizations:
                    ...
                    for view in views:
                        if view == View.COLOR_OVERLAY:
                            row.append(predicted_image.cpu().numpy())
                        ...
                        if view == View.SHAPE_OVERLAY:
                            shape = shape * shape_mask
                            blend = image * (1 - shape_mask) + image * shape_mask * 0.3 + shape * 0.7 * shape_mask
                            row.append(blend.cpu().numpy())
                        if view == View.HEATMAP:
                            t = image.cpu()
                            f = predicted_image.cpu()
