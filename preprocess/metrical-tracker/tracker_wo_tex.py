...
class Tracker(object):
    ...
    def save_checkpoint(self, frame_id):
        ...
        frame = {
            'flame': {
                ...
                'neck': self.neck.clone().detach().cpu().numpy(),                
            },
            'camera': {
        ...
        trimesh.Trimesh(faces=f, vertices=v, process=False).export(os.path.join(self.mesh_folder, f'{frame_id}.ply'))
        # torch.save(frame, f'{self.checkpoint_folder}/{frame_id}.frame')
        torch.save(frame, os.path.join(self.checkpoint_folder, f'{frame_id}.frame'))
    ...
    def clone_params_initialization(self):
        params = [
            ...
            # {'params': [nn.Parameter(self.sh.clone())], 'lr': 0.01, 'name': ['sh']},            # (1, 9, 3)
    ...
    def optimize_camera(self, batch, steps=1000):
        ...
        for k in t:
            ...
            _, lmk68, lmkMP = self.flame(cameras=torch.inverse(self.cameras.R), shape_params=self.shape, expression_params=self.exp, neck_pose_params=self.neck, eye_pose_params=self.eyes, jaw_pose_params=self.jaw)
    ...
    def optimize_video(self):
        ...
        for i in list(range(self.frame, len(self.dataset))):
            ...
            if type(batch) is torch.Tensor:
                continue
            fit_one = self.optimize_frame(batch)
            self.frame += 1
            total_time += fit_one
            print(f"[TIME FOR FITTING ONE FRAME]: {fit_one:.7f}")
        
        json_data = [{'fitting time': total_time}]
        with open(os.path.join(self.save_folder, 'fitting_time.json'), 'w') as fp:
    ...
    def prepare_data(self):
        ...
        self.dataloader = DataLoader(self.dataset, batch_size=256, num_workers=0, shuffle=False, pin_memory=True, drop_last=False)
    ...
    def initialize_tracking(self):
        ...
        for i, j in enumerate(keyframes):   # 用关键帧优化全局的颜色和shape参数
