...
try:
    ...
    from avatar_utils.pytorch3d_load_obj import load_obj

FLAME_MESH_PATH = "flame_model/assets/flame/head_template_mesh.obj"

# 包含static和dynamic的所有indices
FLAME_LMK_PATH = "flame_model/assets/flame/landmark_embedding_with_eyes.npy"
FLAME_MEDIAPIPE_LMK_PATH ="flame_model/assets/mediapipe/mediapipe_landmark_embedding.npz"
...
L_EYELID_PATH = "flame_model/assets/blendshapes/l_eyelid.npy"
R_EYELID_PATH = "flame_model/assets/blendshapes/r_eyelid.npy"
...
def face_vertices(vertices, faces):
    ...
    return vertices[faces.long()]   # (nb, nf, 3, 3)
...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "v_template", to_tensor(to_np(flame_model.v_template * factor), dtype=self.dtype)
        )
        ...
        self.register_buffer('l_eyelid', torch.from_numpy(np.load(L_EYELID_PATH)).to(self.dtype))
        self.register_buffer('r_eyelid', torch.from_numpy(np.load(R_EYELID_PATH)).to(self.dtype))
        
        # NOTE: new added
        mediapipe_lmk_embedding = np.load(FLAME_MEDIAPIPE_LMK_PATH, allow_pickle=True, encoding='latin1')
        
        self.mediapipe_idx = mediapipe_lmk_embedding['landmark_indices'].astype(int)
        
        self.register_buffer(
            'mp_lmk_faces_idx', 
            torch.from_numpy(mediapipe_lmk_embedding['lmk_face_idx'].astype(int)).to(torch.int64)
        )
        
        self.register_buffer(
            'mp_lmk_bary_coords', 
            torch.from_numpy(mediapipe_lmk_embedding['lmk_b_coords']).to(self.dtype).float()
        )
        ...
        self.register_buffer(
            "full_lmk_faces_idx",
            torch.tensor(lmk_embeddings["full_lmk_faces_idx"], dtype=torch.long),   # (1, 70)
        )
        self.register_buffer(
            "full_lmk_bary_coords",
            torch.tensor(lmk_embeddings["full_lmk_bary_coords"], dtype=self.dtype), # (1, 70, 3)
        )
        ...
        vertex_uvs = aux.verts_uvs          # (nv, 2)
        face_uvs_idx = faces.textures_idx  # index into verts_uvs, (nf, 3)
    ...
    def add_teeth(self):
        ...
        vid_lip_outside_ring_upper = self.mask.get_vid_by_region(['lip_outside_ring_upper'], keep_order=True)       # (15)

        vid_lip_outside_ring_lower = self.mask.get_vid_by_region(['lip_outside_ring_lower'], keep_order=True)       # (15)

        v_lip_upper = self.v_template[vid_lip_outside_ring_upper]       # (15, 3)
        v_lip_lower = self.v_template[vid_lip_outside_ring_lower]       # (15, 3)

        # construct vertices for teeth
        mean_dist = (v_lip_upper - v_lip_lower).norm(dim=-1, keepdim=True).mean()       # 上下唇外环的对应点距离取平均
        v_teeth_middle = (v_lip_upper + v_lip_lower) / 2                                # 上下唇外环的对应点的中点
        v_teeth_middle[:, 1] = v_teeth_middle[:, [1]].mean(dim=0, keepdim=True)         # opencv坐标系下，down(Y)轴坐标取所有中点的Y轴坐标的均值，牙齿在同一水平线上？
        ...
        v_teeth_middle[:, 2] -= mean_dist * 1.5  # how far the teeth are from the lips  # opencv坐标系下，牙齿在嘴巴里面，相对于唇外环的front(Z)轴坐标需要减掉一定的距离

        # upper, front
        v_teeth_upper_edge = v_teeth_middle.clone() + torch.tensor([[0, mean_dist, 0]]) * 0.1   # 上排牙齿的下边缘
        v_teeth_upper_root = v_teeth_upper_edge + torch.tensor([[0, mean_dist, 0]]) * 2  # scale the height of teeth，上排牙齿的牙龈根

        # lower, front
        v_teeth_lower_edge = v_teeth_middle.clone() - torch.tensor([[0, mean_dist, 0]]) * 0.1   # 下排牙齿的上边缘
        ...
        v_teeth_lower_root = v_teeth_lower_edge - torch.tensor([[0, mean_dist, 0]]) * 2  # scale the height of teeth，下排牙齿的牙龈根

        # thickness = mean_dist * 0.5
        thickness = mean_dist * 1.      # 牙齿的厚度(Z轴)
        # upper, back
        v_teeth_upper_root_back = v_teeth_upper_root.clone()
        v_teeth_upper_edge_back = v_teeth_upper_edge.clone()
        v_teeth_upper_root_back[:, 2] -= thickness  # how thick the teeth are，上排牙齿根部的背面（口腔内侧）
        v_teeth_upper_edge_back[:, 2] -= thickness  # how thick the teeth are，上排牙齿下边缘的背面

        # lower, back
        v_teeth_lower_root_back = v_teeth_lower_root.clone()
        v_teeth_lower_edge_back = v_teeth_lower_edge.clone()
        v_teeth_lower_root_back[:, 2] -= thickness  # how thick the teeth are，下排牙齿根部的背面（口腔内侧）
        v_teeth_lower_edge_back[:, 2] -= thickness  # how thick the teeth are，下排牙齿上边缘的背面
        ...
        self.l_eyelid = torch.cat([self.l_eyelid, torch.zeros_like(v_teeth)], dim=0)
        self.r_eyelid = torch.cat([self.r_eyelid, torch.zeros_like(v_teeth)], dim=0)
        vid_teeth_upper_root = torch.arange(0, 15) + num_verts_orig         # 新增牙齿网格各个部分的编号
        ...
        self.mask.v.left_half = torch.cat([     # 合并左边的牙齿？
        ...
        self.mask.v.right_half = torch.cat([    # 合并右边的牙齿？
        ...
        u = torch.linspace(0.62, 0.38, 15)          # 15个水平分布的u坐标（对应上下前后牙齿沿着x轴的15个点）
        v = torch.linspace(1-0.0083, 1-0.0425, 7)   # 8个垂直分布的v坐标（对应上牙前根、上牙前边缘、上牙后根、上牙后边缘、下牙前根、下牙前边缘、下牙后根、下牙后边缘）
        # v = v[[0, 2, 1, 1]]
        # v = v[[0, 3, 1, 4, 3, 2, 6, 5]]
        ...
        num_verts_uv_orig = self.verts_uvs.shape[0]     # verts_uvs的个数可以大于verts个数（某些顶点在不同的face中有不同的颜色）
        num_verts_uv_teeth = uv.shape[0]
        self.verts_uvs = torch.cat([self.verts_uvs, uv], dim=0)

        # shapedirs copy from lips, shape系数可以影响牙齿的变形，expr系数不影响牙齿的变形
        self.shapedirs = torch.cat([self.shapedirs, torch.zeros_like(self.shapedirs[:num_verts_teeth])], dim=0)
        shape_dirs_mean = (self.shapedirs[vid_lip_outside_ring_upper, :, :self.n_shape_params] + self.shapedirs[vid_lip_outside_ring_lower, :, :self.n_shape_params]) / 2   # 唇外环上下侧对应的shape基取平均
        ...
        # posedirs set to zero, pose系数不影响牙齿的变形
        ...
        self.lbs_weights[vid_teeth_upper, 1] += 1  # move with neck，上排牙与neck关节刚性链接
        self.lbs_weights[vid_teeth_lower, 2] += 1  # move with jaw，下排牙yujaw关节刚性连接
        ...
        # 通过textures_idx找到每个face对应三个顶点在verts_uvs上的索引
    ...
    def forward(
        ...
        if eyelids is not None:
            vertices = vertices + self.r_eyelid.expand(batch_size, -1, -1) * eyelids[:, 1:2, None]
            vertices = vertices + self.l_eyelid.expand(batch_size, -1, -1) * eyelids[:, 0:1, None]
        ...
        if return_landmarks:
            ...
            mp_lmk_faces_idx = self.mp_lmk_faces_idx.unsqueeze(dim=0).expand(bz, -1).contiguous()
            mp_lmk_bary_coords = self.mp_lmk_bary_coords.unsqueeze(dim=0).expand(bz, -1, -1).contiguous()
            
            mp = vertices2landmarks(
                vertices, 
                self.faces, 
                mp_lmk_faces_idx, 
                mp_lmk_bary_coords
...
class FlameMask(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.faces = faces
        self.faces_t = faces_t  # texture_idx
