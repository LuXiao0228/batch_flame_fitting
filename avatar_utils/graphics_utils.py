...
def getProjectionMatrix(znear, zfar, tanfovx_l, tanfovx_r, tanfovy_t, tanfovy_b):
    tanFovY_t = tanfovy_t
    tanFovY_b = tanfovy_b
    tanFovX_l = tanfovx_l
    tanFovX_r = tanfovx_r
    ...
    top = tanFovY_t * znear
    bottom = -tanFovY_b * znear
    right = tanFovX_r * znear
    left = -tanFovX_l * znear
...
def compute_face_orientation(verts, faces, return_scale=False):
    ...
    if return_scale:
        s0 = length(v1 - v0)
        s1 = dot(a2, (v2 - v0)).abs()
        scale = (s0 + s1) / 2   # 某条边长及其垂线的平均长度 ==> 论文中的 k
