...
import numpy as np
from avatar_utils import vector_ops as V
from avatar_utils.cameras import MiniCam
...
class NVDiffRenderer(torch.nn.Module):
    ...
    def render_from_camera(
        self, verts, faces, cam: MiniCam, background_color=[1., 1., 1.], textures_idx=None, verts_uvs=None, tex=None, 
        face_colors=None,
    ):
        """
        ...
        if self.use_opengl:
            ...
            return self.render_mesh(verts, faces, RT, full_proj, image_size, background_color, textures_idx, verts_uvs, tex, face_colors)
        else:
            ...
            output = self.render_mesh(verts, faces, RT, full_proj, image_size, background_color, textures_idx, verts_uvs, tex, face_colors)
            for k, v in output.items():
                output[k] = F.interpolate(v.permute(0, 3, 1, 2), (cam.image_height, cam.image_width), mode='bilinear').permute(0, 2, 3, 1)
            return output
    
    def render_mesh(
        self, verts, faces, RT, full_proj, image_size, background_color=[1., 1., 1.], textures_idx=None, verts_uvs=None, tex=None, 
        face_colors=None,
    ):
        """
        Renders meshes into RGBA images
        ...
        if textures_idx is not None and verts_uvs is not None and tex is not None:
            texc, texd = dr.interpolate(verts_uvs[None, ...], rast_out, textures_idx, rast_db=rast_out_db, diff_attrs='all')
            albedo = dr.texture(tex[None, ...], texc, texd, filter_mode='linear-mipmap-linear', max_mip_level=4)
